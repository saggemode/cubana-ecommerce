...
import { Banner, Brand } from "@prisma/client";
...
export const BannerForm: React.FC<ProductFormProps> = ({
  initialData,
  bannerId,
}) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/banners/${bannerId}`, values);
      } else {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/banner/${params.productId}`);
      router.refresh();
